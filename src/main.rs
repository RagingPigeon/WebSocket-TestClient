...
async fn main() {
    ...
    let mut send_new_message_response_received: bool = false;
    ...
    let (mut send_new_message_socket, send_response) = connect_async("ws://localhost:7878/send").await.expect("Failed to connect");

    let (mut send_write, mut send_read) = send_new_message_socket.split();

    //======================================================================
    // Send New Message Endpoint    

    // Build the request to send to chatsurfer-connect.
    let new_message = Message::Text(build_new_message_request());
    
    // Send the request.
    send_write.send(new_message).await;

    // Wait until we receive a response.
    match send_read.next().await {
        Some(response) => {
